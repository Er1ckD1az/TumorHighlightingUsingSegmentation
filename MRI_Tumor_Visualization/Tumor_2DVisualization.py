...
def segment_tumor(mri_data, threshold_factor=2.0):
    ...
    #We compute the threshold using Otsu's method and adjust with the factor
    ...
    #Find the largest component (we assume it's the tumor)
    if num_features > 1:
...
def display_multi_view(mri_data, tumor_mask):
    ...
    mask_z[:, :, 0] = 1.0 
    mask_z[:, :, 3] = tumor_mask[:, :, best_z] * 0.7  
    ...
    mask_y[:, :, 0] = 1.0  
    mask_y[:, :, 3] = tumor_mask[:, best_y, :].T * 0.7  
    ...
    mask_x[:, :, 0] = 1.0  
    mask_x[:, :, 3] = tumor_mask[best_x, :, :].T * 0.7  
